    ...
    <div calss="table-responsive">
        <table class="table">
            <thead>
                <tr>
                    ...
                    @foreach ($results as $result)
                        <th colspan="{{ count($result['questions']) * 4 }}">{{ $result['nome'] }}</th>
                    @endforeach
                </tr>
                <tr>
                    @foreach ($results as $result)
            ...
            <tbody>
                    ...
                    @foreach ($results as $result)
                        @foreach ($result['questions'] as $questao)
                            @foreach (['A', 'B', 'C', 'D'] as $alt)
                                <td>{{ $questao['alternatives_count'][$alt] }}</td>
                            @endforeach
                        @endforeach
                    @endforeach
                </tr>
                        ...
                        @foreach ($result['questions'] as $questao)
                            @foreach (['A', 'B', 'C', 'D'] as $alt)
                                <td>
                                    ...
                                    @else
                                        <!-- Exibe vazio para alternativas incorretas -->
                                        -
                                    @endif
                                </td>
                            @endforeach
                            ...
                            @foreach (['A', 'B', 'C', 'D'] as $alt)
                                <td>
                                    @if ($alt !== $questao['correct_alternative'])
                                        {{ $questao['alternatives_count'][$alt] }}
                                    @else
                                        <!-- Exibe vazio para a alternativa correta -->
                                        -
                                    @endif
                                </td>
                            @endforeach
                ...
                <!-- Linha de Não Respondeu -->
                <tr>
                    <td>Não Respondeu</td>
                    @foreach ($results as $result)
                        @foreach ($result['questions'] as $questao)
                            @foreach (['A', 'B', 'C', 'D'] as $alt)
                                <td>{{ $questao['alternatives_count'][$alt] === 0 ? '-' : '' }}</td>
                            @endforeach
                        @endforeach
                    @endforeach
                </tr>
            ...
            <tr>
                ...
                @foreach ($results as $result)
                    @foreach ($result['questions'] as $questao)
                        @foreach (['A', 'B', 'C', 'D'] as $alt)
                            <td>
                                {{ ($question['alternatives_count'][$alt] ?? 0) === 0 ? '-' : '' }}
                            </td>
                        @endforeach
                    @endforeach
                @endforeach
